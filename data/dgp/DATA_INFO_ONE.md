# Synthetic Dataset One (`synthetic_one.py`)

This describes the structure produced by [`synthetic_one.py`](/home/shiang/projects/aip-rgreiner/shiang/diversity_by_design/data/dgp/synthetic_one.py) (`synthetic_DGP`) when data are written as chunked `.h5ad` files and loaded with backed `AnnCollection`.

## Output format

`synthetic_DGP(...)` returns:

- `chunk_paths: list[str]`
- `all_affected_masks: list[np.ndarray]` (length `P`, each mask shape `(G,)`)
...
- `ac.shape = (N0 + P*Nk, G)`
- `ac.obs` columns: `["perturbation", "chunk_id"]`
- `ac.var_names`: `gene_0 ... gene_{G-1}`
- `ac.attrs_keys`:
  - `obs`: `["perturbation", "chunk_id"]`
...
- `sample.obs`: includes `perturbation` + `chunk_id`
